...
C...Local variables and data statement.
     ...
     & YMIN,YMAX
      ...
      INTEGER initialPID
      initialPID = K(1,2)
      ! if(initialPID.eq.22 .and. IGAGA.eq.5) then
      !     WRITE(*,*)"IGAGA    = ", IGAGA
      !     WRITE(*,*)"MSTI(1) (Subprocess):", MSTI(1)
      !     WRITE(*,*)"ebeam    = ", ebeam
      !     WRITE(*,*)"K(1,2)PID= ", K(1,2)
      !     WRITE(*,*)"K(2,2)PID= ", K(2,2)
      !     WRITE(*,*)"P(1,1) px= ", P(1,1)
      !     WRITE(*,*)"P(1,2) py= ", P(1,2)
      !     WRITE(*,*)"P(1,3) pz= ", P(1,3)
      !     WRITE(*,*)"P(1,4) E = ", P(1,4)
      !     WRITE(*,*)"P(2,1) px= ", P(2,1)
      !     WRITE(*,*)"P(2,2) py= ", P(2,2)
      !     WRITE(*,*)"P(2,3) pz= ", P(2,3)
      !     WRITE(*,*)"P(2,4) E = ", P(2,4)
      ! endif

      if(ebeam.ne.ebeam_original.and.initialPID.eq.22 
     &      .and.IGAGA.eq.5) then
C          WRITE(*,*)""
C          WRITE(*,*) char(27)//
C     &      "[1;31m", "Beam Energy/Particle has changed before",
C     &      " PYGAGA(5) (cannot be fixed)", char(27)//"[0m"
C          WRITE(*,*) char(27)//
C     &      "[1;31m", "EVENT IS BEING KILLED", char(27)//"[0m"
          ! WRITE(*,*)"ebeam    = ", ebeam
          MSTI(61) = 1
          RETURN
      endif

! Notification of new beam PID (can still generate events under these conditions)
!       if(ebeam.eq.ebeam_original.and.initialPID.eq.22 
!      &      .and.IGAGA.eq.5) then
!           write(*,*) char(27)//"[1;34m", 
!      &      "Beam is a photon but energy is still correct",
!      &      " (ISR corrections not possible - but event will",
!      &      " still generate properly)", char(27)//"[0m"
!       endif
...
C            temp = ((genQ2-minq2)/(2D0*ebeamEnucl*geneprim))-1D0
  ...
  122       if (qedrad.eq.1) then
     ...
     &          (mcRadCor_ThetaBrems.eq.mcRadCor_ThetaBrems)) then
             call Update_Beam_Vars(dble(mcRadCor_Q2True),
     &          dble(mcRadCor_NuTrue),
     &          dble(mcRadCor_EBrems),
     &          dble(mcRadCor_ThetaBrems),
     &          dble(mcRadCor_PhiBrems))

             Y(I)=dble(mcRadCor_NuTrue)/dble(mcSet_EneBeam)
             Q2(I)=dble(mcRadCor_Q2True)
             mcRadCor_YTrue=dble(mcRadCor_NuTrue)/dble(mcSet_EneBeam)
             if (mcRadCor_XTrue > 1.) then
C                write(*,*)"ERROR: mcRadCor_XTrue=", mcRadCor_XTrue
                ! write(*,*)"mcRadCor_Q2True      =", mcRadCor_Q2True
                ! write(*,*)"mcRadCor_YTrue       =", mcRadCor_YTrue
                ! write(*,*)"mcRadCor_EBrems      =", mcRadCor_EBrems
                ! write(*,*)"mcRadCor_ThetaBrems  =", mcRadCor_ThetaBrems
                ! write(*,*)"ebeam                =", ebeam
                ! write(*,*)"K(1,2)               =", K(1,2)
                ! write(*,*)"LOOP                 =", LOOP
                
!     Compute the value of mcRadCor_W before writing it
C                mcRadCor_W = &
C                sqrt(massp**2+(q2true*(1./mcRadCor_XTrue-1.)))
                mcRadCor_W2 = massp**2 + 
     &               (mcRadCor_Q2True * (1. / mcRadCor_XTrue - 1.))

                ! write(*,*)"mcRadCor_W2          =", mcRadCor_W2
                GOTO 121
             endif
           else
...
C...Add on scattered leptons to final state.
  ...
  290 CONTINUE
      ...
      CONTAINS
        ...
        ! Subroutine: SaveGAGA
        ! Description:
        ! Repeats the PYGAGA(1) operation, saving quantities on the incoming
        ! lepton system and calculating ranges for allowed x and Q2 values.
        ! Necessary for updating the beam energy after running PYINKI(0).
        !----------------------------------------------------------------------
        SUBROUTINE SaveGAGA()
            ! Save quantities on incoming lepton system.
            VINT(301)=VINT(1)
            VINT(302)=VINT(2)
            PMS(1)=VINT(303)**2
            IF(MINT(141).EQ.0) PMS(1)=SIGN(VINT(3)**2,VINT(3))
            ...
            PMC(3)=VINT(302)-PMS(1)-PMS(2)
            W2MIN=MAX(CKIN(77),2D0*CKIN(3),2D0*CKIN(5))**2
            ! Calculate range of x and Q2 values allowed in generation.
            DO 101 R=1,2
            PMC(R)=VINT(302)+PMS(R)-PMS(3-R)
            IF(MINT(140+R).NE.0) THEN
                XMIN(R)=MAX(CKIN(59+2*R),EPS)
                XMAX(R)=MIN(CKIN(60+2*R),1D0-2D0*VINT(301)*
     &          SQRT(PMS(R))/PMC(R),1D0-EPS)
                YMIN=MAX(CKIN(71+2*R),EPS)
                YMAX=MIN(CKIN(72+2*R),1D0-EPS)
                IF(CKIN(64+2*R).GT.0D0) XMIN(R)=MAX(XMIN(R),
     &          (YMIN*PMC(3)-CKIN(64+2*R))/PMC(R))
                XMAX(R)=MIN(XMAX(R),(YMAX*PMC(3)-
     &          CKIN(63+2*R))/PMC(R))
                THEMIN=MAX(CKIN(67+2*R),0D0)
                THEMAX=MIN(CKIN(68+2*R),PARU(1))
                IF(CKIN(68+2*R).LT.0D0) THEMAX=PARU(1)
                Q2MIN(R)=MAX(CKIN(63+2*R),XMIN(R)**2*PMS(R)/
     &          (1D0-XMIN(R))+((1D0-XMAX(R))*(VINT(302)-2D0*
     &          PMS(3-R))-2D0*PMS(R)/(1D0-XMAX(R)))*SIN(THEMIN/
     &          2D0)**2,0D0)
                Q2MAX(R)=XMAX(R)**2*PMS(R)/(1D0-XMAX(R))+
     &          ((1D0-XMIN(R))*(VINT(302)-2D0*PMS(3-R))-2D0*
     &          PMS(R)/(1D0-XMIN(R)))*SIN(THEMAX/2D0)**2
                IF(CKIN(64+2*R).GT.0D0) Q2MAX(R)=MIN(CKIN(64
     &          +2*R),Q2MAX(R))
                ! W limits when lepton on one side only.
                IF(MINT(143-R).EQ.0) THEN
                    XMIN(R)=MAX(XMIN(R),(W2MIN-PMS(3-R))/
     &          PMC(R))
                    IF(CKIN(78).GT.0D0) XMAX(R)=MIN(XMAX(R),
     &          (CKIN(78)**2-PMS(3-R))/PMC(R))
                ENDIF
            ENDIF
  101   CONTINUE
            ! W limits when lepton on both sides.
            IF(MINT(141).NE.0.AND.MINT(142).NE.0) THEN
                IF(CKIN(78).GT.0D0) XMAX(1)=MIN(XMAX(1),
     &          (CKIN(78)**2+PMC(3)-PMC(2)*XMIN(2))/PMC(1))
                IF(CKIN(78).GT.0D0) XMAX(2)=MIN(XMAX(2),
     &          (CKIN(78)**2+PMC(3)-PMC(1)*XMIN(1))/PMC(2))
                IF(IABS(MINT(141)).NE.IABS(MINT(142))) THEN
                    XMIN(1)=MAX(XMIN(1),(PMS(1)-PMS(2)+
     &          VINT(302)*(W2MIN-PMS(1)-PMS(2))/(PMC(2)*
     &          XMAX(2)+PMS(1)-PMS(2)))/PMC(1))
                    XMIN(2)=MAX(XMIN(2),(PMS(2)-PMS(1)+
     &          VINT(302)*(W2MIN-PMS(1)-PMS(2))/(PMC(1)*
     &          XMAX(1)+PMS(2)-PMS(1)))/PMC(2))
                ELSE
                    XMIN(1)=MAX(XMIN(1),W2MIN/(VINT(302)*
     &          XMAX(2)))
                    XMIN(2)=MAX(XMIN(2),W2MIN/(VINT(302)*
     &          XMAX(1)))
                ENDIF
            ENDIF
            
        END SUBROUTINE SaveGAGA

        !----------------------------------------------------------------------
        ! Subroutine: Reset_Beam_Vars
        ! Description:
        ! Resets the beam variables to their original values, restoring the initial
        ! beam energy and momentum components to their default state.
        !----------------------------------------------------------------------
        SUBROUTINE Reset_Beam_Vars()
            ! Globals or Accessible Variables
            ! REAL*8 :: ebeam, ebeamEnucl, mcSet_EneBeam
            ! REAL*8 :: masse, massp, pbeam
            
            ! Local Variable
            REAL*8 :: ebeamE, pbeta, pgamma, pbeamE
            
            REAL*8 :: ebE_com, ebx_com, eby_com, ebz_com ! Center-of-Mass kinematics of lepton beam
            REAL*8 :: pbE_com, pbx_com, pby_com, pbz_com ! Center-of-Mass kinematics of target
            ...
            IF(verbose) THEN
                WRITE(*,*)"Calling Reset_Beam_Vars"
                ! Write the current values before reset
                ! WRITE(*,*)""
                ! WRITE(*,*)"Before Reset: ebeam         = ", ebeam
                ! WRITE(*,*)"Before Reset: ebeamEnucl    = ", ebeamEnucl
                ! WRITE(*,*)"Before Reset: mcSet_EneBeam = ", mcSet_EneBeam
                ! WRITE(*,*)"Before Reset: pbeam         = ", pbeam
                ! WRITE(*,*)"K(1,2)PID= ", K(1,2)
                ! WRITE(*,*)"K(2,2)PID= ", K(2,2)
                ! WRITE(*,*)"P(1,1) px= ", P(1,1)
                ! WRITE(*,*)"P(1,2) py= ", P(1,2)
            ...
            IF ((K(1,2).EQ.11).AND.(K(2,2).EQ.2212)) THEN
                IF(verbose) WRITE(*,*)"NORMAL (e+p) EVENT"
                ...
                !     WRITE(*,*)""
                ! ENDIF
                IF(verbose) WRITE(*,*)"Running SaveGAGA" ! The equivalent of running PYGAGA(1) to save the changes made with PYINKI(0)
                Call SaveGAGA()
                IF(verbose) WRITE(*,*)"Ran SaveGAGA"
            ELSEIF(verbose) THEN
                WRITE(*,*)"Not e+p event (anymore)"
                ! WRITE(*,*)"Initial event particles have changed"
            ENDIF
        END SUBROUTINE Reset_Beam_Vars

        !----------------------------------------------------------------------
        ! Subroutine: Update_Beam_Vars
        ! Description:
        ! Updates the beam variables based on detected ISR. Determines ISR status
        ! and modifies beam variables accordingly.
        !----------------------------------------------------------------------
        SUBROUTINE Update_Beam_Vars(q2ISR, nuISR,
     &          gEnergy_rad, gTheta_rad, gPhi_rad)
            ! Inputs
            REAL*8, INTENT(IN) :: q2ISR, nuISR
            REAL*8, INTENT(IN) :: gEnergy_rad, gTheta_rad, gPhi_rad

            ! Local Variables
            LOGICAL :: isISR, verbose
            REAL*8 :: ebeam_modified, px_new, py_new, pz_new
            REAL*8 :: ebeamE, pbeta, pgamma, pbeamE
            REAL*8 :: etheta_scat, denom_factor, cosine_term
            REAL*8 :: radgamEnucl, radgamE, radgamp

            ! Initialize default ISR status
            isISR   = .FALSE.
            verbose = .FALSE.

            IF (verbose) THEN
                WRITE(*,*)""
                WRITE(*,*)"Calling Update_Beam_Vars"
            END IF

            ! Check if photon momentum is undefined (no photon)
!             IF ((gEnergy_rad.EQ. 0.0D0).OR.
!      &          (gTheta_rad.EQ. 0.0D0).OR.
!      &          (gPhi_rad.EQ. 0.0D0)) THEN
            IF (mcRadCor_EBrems.LE.0.) THEN ! The inverse of the condition that is used to write the radiated photon to the output file (i.e., if this statement is true, then no photon would be recorded by the pythia6-eic.f script)
                IF (verbose) THEN
                    WRITE(*,*)""
                    WRITE(*,*)"No photon momentum detected"
                    WRITE(*,*)""
                END IF
                RadState = 0
                RETURN
...
!             ! Check if incoming beam/target are still the electron/proton
!             IF ((K(1,2).NE.11).OR.(K(2,2).NE.2212)) THEN
!                 IF (verbose) THEN
!                     WRITE(*,*)""
!                     WRITE(*,*)"Incoming particle is not the",
!      &          " electron beam OR the target is not a proton"
!                     WRITE(*,*)"K(1,2) = ", K(1,2)
!                     WRITE(*,*)"K(2,2) = ", K(2,2)
!                     WRITE(*,*)""
!                 END IF
...
!             END IF
            ...
            ! Calculate scattered electron polar angle in parts
            denom_factor = ebeam_original * (ebeam_original - nuISR)
            cosine_term = (q2ISR / (2.0D0 * denom_factor))
            etheta_scat = ACOS(1.0D0 - cosine_term)

            ! Determine if ISR condition is met
            isISR = (gTheta_rad < ABS(etheta_scat - gTheta_rad))

            ! If ISR, modify beam properties
            IF (isISR) THEN
                ! Check if incoming beam/target are still the electron/proton
                IF ((K(1,2).NE.11).OR.(K(2,2).NE.2212)) THEN
                    IF (verbose) THEN
                        ...
                        WRITE(*,*)"Incoming particle is not the",
     &          " electron beam OR the target is not a proton",
     &          " (Cannot Modify Beam Energy for ISR)"
                        WRITE(*,*)"K(1,2) = ", K(1,2)
                        WRITE(*,*)"K(2,2) = ", K(2,2)
                        WRITE(*,*)""
                    END IF
                ...
                IF (verbose) THEN
                    WRITE(*,*)"ISR detected."
                    WRITE(*,*)"Before: ebeam         = ", ebeam
                    ! WRITE(*,*)"Before: ebeamEnucl    = ", ebeamEnucl
                    ! WRITE(*,*)"Before: mcSet_EneBeam = ", mcSet_EneBeam
                    ! WRITE(*,*)"Before:             s = ", s
                END IF
                ...
                pbeta       = pbeam / pbeamE
                pgamma      = pbeamE / massp
                radgamEnucl = sqrt(dplabg(1)**2 + dplabg(2)**2 + 
     &          dplabg(3)**2)
                radgamE     =  pgamma * radgamEnucl - pgamma * pbeta 
     &          * dplabg(3)
                radgamp     = -pgamma * pbeta * radgamEnucl + pgamma 
     &          * dplabg(3)
                
                IF (verbose) THEN
                    WRITE(*,*)"gEnergy_rad, gTheta_rad, gPhi_rad = ", 
     &          gEnergy_rad, ", ", gTheta_rad, ", ", gPhi_rad
                    WRITE(*,*)"dplabg                            = ", 
     &          dplabg(1), ", ", dplabg(2), ", ", dplabg(3)
                    WRITE(*,*)"Photon Kinematics                 = ", 
     &          dplabg(1), ", ", dplabg(2), ", ", radgamp, ", ", radgamE
                    WRITE(*,*)"P(1,1) px= ", P(1,1)
                    WRITE(*,*)"P(1,2) py= ", P(1,2)
                    WRITE(*,*)"P(1,3) pz= ", P(1,3)
                    WRITE(*,*)"P(1,4) E = ", P(1,4)
                ...
                ! Adjust beam momentum and energy due to ISR
                ebeamE = SQRT(ebeam_original**2 + masse**2)
                px_new = -dplabg(1)
                py_new = -dplabg(2)
                pz_new = -ebeam_original + radgamp
                ebeam_modified = ebeamE - radgamE
                
                ! Update global beam properties
                ebeam         = pz_new
                ebeamE        = ebeam_modified
                ! ebeamE        = SQRT(ebeam**2 + masse**2)
                ebeamEnucl    = pgamma * ebeamE - pgamma * pbeta * pz_new
                mcSet_EneBeam = SNGL(ebeamEnucl)
                ! s = pbeamE**2 + 2.0*pbeamE*ebeamE + ebeamE**2 - (pbeam - ebeam)**2

                ! Target is defined in positive z
                P(2,1)=0.0  
                P(2,2)=0.0  
                P(2,3)=pbeam
                P(2,4)=pbeamE
                ! Beam/lepton is defined in negative z
                P(1,1)=px_new
                P(1,2)=py_new
                P(1,3)=pz_new
                P(1,4)=ebeamE
                ...
                IF (verbose) THEN
                    ...
                    WRITE(*,*)"P(2,3) pz= ", P(2,3)
                    WRITE(*,*)"P(2,4) E = ", P(2,4)
                    WRITE(*,*)"CALLING PYINKI(0)"
                ENDIF
                CALL PYINKI(0)
                IF (verbose) THEN
                    WRITE(*,*)"After Updated PYINKI(0):"
                    WRITE(*,*)"P(1,1) px= ", P(1,1)
                    WRITE(*,*)"P(1,2) py= ", P(1,2)
                    WRITE(*,*)"P(1,3) pz= ", P(1,3)
                    WRITE(*,*)"P(1,4) E = ", P(1,4)
                    ...
                    WRITE(*,*)"P(2,3) pz= ", P(2,3)
                    WRITE(*,*)"P(2,4) E = ", P(2,4)
                    WRITE(*,*)"Running SaveGAGA" ! The equivalent of running PYGAGA(1) to save the changes made with PYINKI(0)
                END IF
                ...
                IF (verbose) THEN
                    WRITE(*,*)"Ran SaveGAGA"
                    WRITE(*,*)"After Update:  ebeam         = ", ebeam
                    ! WRITE(*,*)"After Update:  ebeamEnucl    = ", ebeamEnucl
                    ! WRITE(*,*)"After Update:  mcSet_EneBeam = ", mcSet_EneBeam
                    ! WRITE(*,*)"After Update:              s = ", s
                    WRITE(*,*)"P(1,1) px= ", P(1,1)
                    WRITE(*,*)"P(1,2) py= ", P(1,2)
                    WRITE(*,*)"P(1,3) pz= ", P(1,3)
                    WRITE(*,*)"P(1,4) E = ", P(1,4)
            ...
            ELSE
                RadState = 2
                IF (verbose) THEN
                    WRITE(*,*) "No ISR detected. (Event is FSR)"
                    WRITE(*,*)""
                END IF
            END IF
        END SUBROUTINE Update_Beam_Vars
